...
const ui_renderer_system = @import("../systems/ui_renderer_system.zig");
...
var ui_renderer_sys: *ui_renderer_system.SystemState = undefined;
...
pub fn createSystems(gameloop_context: anytype, system_context: *util.Context) void {
    ...
    ui_renderer_sys = try ui_renderer_system.create(
        IdLocal.initFormat("ui_renderer_system_{}", .{0}),
        std.heap.page_allocator,
        gameloop_context.ecsu_world,
        gameloop_context.ui_buffer_indices,
    );
}
...
pub fn destroySystems() void {
    ...
    defer ui_renderer_system.destroy(ui_renderer_sys);
}
