...
[numthreads(CULL_INSTANCES_THREADS_COUNT, 1, 1)] void main(uint DTid : SV_DispatchThreadID)
{
    ...
    bool isVisible = FrustumCull(instance.boundsOrigin, instance.boundsExtents, instance.world, g_Frame.viewProj);
    ...
    if (isVisible)
    {
        if (g_CullInstancesParams.shadowPass == 0)
        {
            if (instance.flags & (1 << 1))
            {
                ...
                DrawBoundingSphere(instance.boundsOrigin, instance.boundsExtents, instance.world);
            }
        }

        ByteAddressBuffer mesh_buffer = ResourceDescriptorHeap[g_Frame.meshesBufferIndex];
        ByteAddressBuffer renderableBuffer = ResourceDescriptorHeap[g_Frame.renderableBufferIndex];

        for (uint ri = 0; ri < instance.renderableItemCount; ri++)
        {
            RenderableItem renderableItem = renderableBuffer.Load<RenderableItem>((instance.renderableItemIndex + ri) * sizeof(RenderableItem));
            ...
            float3 center = mul(float4(renderableItem.localBoundsOrigin, 1.0), instance.world).xyz;
            float distanceToCamera = max(0.01, length(center - g_Frame.cameraPosition.xyz));
            bool isLodVisible = distanceToCamera >= renderableItem.screenPercentageMin && distanceToCamera <= renderableItem.screenPercentageMax;
            isLodVisible &= FrustumCull(renderableItem.localBoundsOrigin, renderableItem.localBoundsExtents, instance.world, g_Frame.viewProj);

            if (isLodVisible)
            {
                Mesh mesh = mesh_buffer.Load<Mesh>(renderableItem.meshIndex * sizeof(Mesh));
                ...
                for (uint i = 0; i < meshletsToAddCount; i++)
                {
                    ...
                    meshlet.instanceId = instanceIndex;
                    meshlet.meshIndex = renderableItem.meshIndex;
                    meshlet.materialIndex = renderableItem.materialIndex;
