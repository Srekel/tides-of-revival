...
void SampleTerrainLayer(uint layer_index, float2 uv, float3 N, float3 V, out float3 albedo, out float3 normal, out float3 arm, out float height) {
    ...
    height = heightTexture.Sample(Get(bilinearRepeatSampler), uv).r;
}
