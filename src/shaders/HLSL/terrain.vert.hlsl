...
VSOutput VS_MAIN(VSInput Input, uint instance_id : SV_InstanceID)
{
    ...
    float3 normal = float3(
        height - SampleLvlTex2D(heightmap, Get(bilinearClampSampler), Out.UV + float2(1.0f / 65.0f, 0.0f), 0).r,
        vertex_spacing,
        height - SampleLvlTex2D(heightmap, Get(bilinearClampSampler), Out.UV + float2(0.0f, 1.0f / 65.0f), 0).r
    );
