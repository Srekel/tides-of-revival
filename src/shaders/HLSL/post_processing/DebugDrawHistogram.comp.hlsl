// Based on https://github.com/microsoft/DirectX-Graphics-Samples/blob/master/MiniEngine/Core/Shaders/DebugDrawHistogramCS.hlsl
...
RWTexture2D<float3> ColorBuffer : register(u0, UPDATE_FREQ_PER_FRAME);

groupshared uint gs_hist[256];

[numthreads(256, 1, 1)] void main(uint GI : SV_GroupIndex, uint3 DTid : SV_DispatchThreadID)
{
