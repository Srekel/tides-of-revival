// Based on https://github.com/microsoft/DirectX-Graphics-Samples/blob/master/MiniEngine/Core/Shaders/BloomExtractAndDownsampleHdrCS.hlsl
...
[numthreads(8, 8, 1)] void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    float3 color1 = SourceTex.SampleLevel(g_linear_clamp_edge_sampler, uv + float2(-offset.x, -offset.y), 0);
    float3 color2 = SourceTex.SampleLevel(g_linear_clamp_edge_sampler, uv + float2(offset.x, -offset.y), 0);
    float3 color3 = SourceTex.SampleLevel(g_linear_clamp_edge_sampler, uv + float2(-offset.x, offset.y), 0);
    float3 color4 = SourceTex.SampleLevel(g_linear_clamp_edge_sampler, uv + float2(offset.x, offset.y), 0);
