...
Texture2D<float3> BloomBuf : register(t0, UPDATE_FREQ_PER_FRAME);
RWTexture2D<float3> Result1 : register(u0, UPDATE_FREQ_PER_FRAME);
RWTexture2D<float3> Result2 : register(u1, UPDATE_FREQ_PER_FRAME);
RWTexture2D<float3> Result3 : register(u2, UPDATE_FREQ_PER_FRAME);
RWTexture2D<float3> Result4 : register(u3, UPDATE_FREQ_PER_FRAME);
...
[numthreads(8, 8, 1)] void main(uint GI : SV_GroupIndex, uint3 DTid : SV_DispatchThreadID)
{
    ...
    float3 avgPixel = BloomBuf.SampleLevel(g_linear_clamp_edge_sampler, centerUV, 0.0f);
    g_Tile[GI] = avgPixel;
