    ...
    pub fn getHeightWorld(self: HeightmapOutputData, world_x: anytype, world_y: anytype) f32 {
        if (@typeInfo(@TypeOf(world_x)) == .Float) {
            const height = self.getHeight(
                @intFromFloat(i32, @floor(world_x)),
                @intFromFloat(i32, @floor(world_y)),
            );
            const height_0_1 = @floatFromInt(f32, height) / @floatFromInt(f32, std.math.maxInt(u16));
            ...
            if (heightmapOpt != null) {
                ...
                while (y < patch_width) : (y += 1) {
                    ...
                    while (x < patch_width) : (x += 1) {
                        ...
                        var height_sample: f32 = data.noise.noise2(@floatFromInt(f32, x_world) * config.noise_scale_xz, @floatFromInt(f32, y_world) * config.noise_scale_xz) * 0.5 + 0.5;
                        ...
                        heightmap[x + y * patch_width] = @intFromFloat(HeightmapHeight, height_sample * 65535);
